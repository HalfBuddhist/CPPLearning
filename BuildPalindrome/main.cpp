...
void SuffixSort(int n) {
    ...
    //sa为suffix array
    sort(SA, SA + n, smaller_first_char); //将sa按照位置所对应字母的升序来排, sa此时存放的是排序后的字母所在的索引

    for (int i = 0; i < n; ++i) {
        bh[i] = i == 0 || txt[SA[i]] != txt[SA[i - 1]]; //存入对于前一个字符的不等性, 即相邻字符的相等性
        b2h[i] = false;
    }
    ...
    for (int h = 1; h < n; h <<= 1) {
        ...
        for (int i = 0, j; i < n; i = j) {
            j = i + 1;
            while (j < n && !bh[j]) j++;//开始的第一个不等位置
            next_gen[i] = j; //next_gen 存放的是下一个不等字符的位置
            buckets++;
        }
        if (buckets == n) break; //是否有相同字符
        for (int i = 0; i < n; i = next_gen[i]) {
            cnt[i] = 0;
            for (int j = i; j < next_gen[i]; ++j) {
                iSA[SA[j]] = i; //与其字符相同的排序后最前的位置
            }
        }
        cnt[iSA[n - h]]++; ////?
        b2h[iSA[n - h]] = true; ///?
